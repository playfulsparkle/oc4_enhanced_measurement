...
class PsEnhancedMeasurement extends \Opencart\System\Engine\Controller
{
    ...
    public function eventCatalogViewProductCategoryBefore(string &$route, array &$args, string &$template): void
    {
        ...
        if ($category_info) {
            ...
            foreach ($products as $key => $product_info) {
                ...
                $items[$product_info['product_id']] = $item;
            }
    ...
    public function eventCatalogViewProductSearchBefore(string &$route, array &$args, string &$template): void
    {
        ...
        if ($search || $tag) {
            ...
            if (isset($this->request->get['search'])) {
                $item_list_id = $this->_formatListId($this->language->get('heading_title') . ' - ' . $this->request->get['search']);
                $item_list_name = $this->language->get('heading_title') . ' - ' . $this->request->get['search'];
            } elseif (isset($this->request->get['tag'])) {
                $item_list_id = $this->_formatListId($this->language->get('heading_title') . ' - ' . $this->language->get('heading_tag') . $this->request->get['tag']);
                $item_list_name = $this->language->get('heading_title') . ' - ' . $this->language->get('heading_tag') . $this->request->get['tag'];
            } else {
                $item_list_id = $this->_formatListId($this->language->get('heading_title'));
