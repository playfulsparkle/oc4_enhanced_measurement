...
class PsEnhancedMeasurement extends \Opencart\System\Engine\Controller
{
    ...
    public function eventCatalogViewProductSearchBefore(string &$route, array &$args, string &$template): void
    {
        ...
        if ($search || $tag) {
            if (isset($this->request->get['search'])) {
                ...
                $item_list_id = $this->formatListId($this->language->get('heading_title') . ' - ' . $this->language->get('heading_tag') . $this->request->get['tag']);
                $item_list_name = $this->language->get('heading_title') . ' - ' . $this->language->get('heading_tag') . $this->request->get['tag'];
            } else {
                $item_list_id = $this->formatListId($this->language->get('heading_title'));
                $item_list_name = $this->language->get('heading_title');
            }
    ...
    public function eventCatalogViewCheckoutCartBefore(string &$route, array &$args, string &$template): void
    {
        ...
        if ($products) {
            ...
            if ($items) {
                if ($item_price_tax) {
                    $total = $this->tax->calculate($product_info['total'], $product_info['tax_class_id'], $this->config->get('config_tax'));
                } else {
                ...
                $args['ps_view_cart'] = json_encode($ps_view_cart, JSON_PRETTY_PRINT | JSON_NUMERIC_CHECK);
            }
        }
        ...
        if ($products) {
            ...
            foreach ($products as $index => $product_info) {
                ...
                if ($item_price_tax) {
                    ...
                    $one_price = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
                } else {
                    $price = $product_info['price'] * $product_info['quantity'];

                    $one_price = $product_info['price'];
                }
