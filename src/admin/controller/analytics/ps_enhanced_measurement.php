...
class PsEnhancedMeasurement extends \Opencart\System\Engine\Controller
{
     ...
     */
    const GITHUB_REPO_URL = 'https://github.com/playfulsparkle/oc4_enhanced_measurement';
    ...
    public function index(): void
    {
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('extension/ps_enhanced_measurement/analytics/ps_enhanced_measurement', 'user_token=' . $this->session->data['user_token'] . '&store_id=' . $this->request->get['store_id'], true),
        ];
    ...
    public function send_refund(): void
    {
        ...
        $params = [
            ...
            'value' => $this->currency->format($sub_total + $total_fees, $config_currency, 0, false),
            'coupon' => $product_coupon ? $product_coupon : '',
            'shipping' => $this->currency->format($shipping, $config_currency, 0, false),
            'tax' => $this->currency->format($total - $sub_total, $config_currency, 0, false),
    ...
    public function eventAdminControllerSaleOrderAddHistoryAfter(&$route, &$args, &$output)
    {
        ...
        if (empty($config_currency)) {
            $config_currency = $order_info['currency_code'];
        }
        ...
        if ($working_lead === 1) {
            $params = [
                'currency' => $config_currency,
                'value' => $this->currency->format($order_info['total'], $config_currency, 0, false),
